...
public final class StationDebugCommand {
    ...
    private static final ResourceLocation DEFAULT_VILLAGE = ResourceLocation.fromNamespaceAndPath("minecraft", "village_plains");
    ...
    public static void register(final CommandDispatcher<CommandSourceStack> dispatcher) {
        final LiteralArgumentBuilder<CommandSourceStack> root = Commands.literal("hngstation").requires(source -> source.hasPermission(0));
        ...
        root.then(Commands.literal("list").executes(ctx -> {
            ...
            if (stations.isEmpty()) {
                source.sendSuccess(() -> Component.literal("[hng-station] no stations captured yet in " + level.dimension().location()), false);
                return 0;
            }
            source.sendSuccess(() -> Component.literal("[hng-station] " + stations.size() + " station(s) captured in " + level.dimension().location() + ":"), false);
            for (final StationRecord s : stations) source.sendSuccess(() -> Component.literal("  id=" + s.stationId() + " type=" + s.typeOrdinal() + " exits=" + s.exitCount() + " @ " + s.pos().getX() + "," + s.pos().getY() + "," + s.pos().getZ()), false);
        ...
        root.then(Commands.literal("nearest").executes(ctx -> {
            ...
            source.sendSuccess(() -> Component.literal(String.format("[hng-station] nearest: id=%d type=%d exits=%d @ %d,%d,%d (%.1f blocks away)", nearest.stationId(), nearest.typeOrdinal(), nearest.exitCount(), nearest.pos().getX(), nearest.pos().getY(), nearest.pos().getZ(), dist)), false);
            return 1;
        }));
        ...
        root.then(Commands.literal("tp").executes(ctx -> {
            ...
            source.sendSuccess(() -> Component.literal(String.format("[hng-station] teleported to station id=%d @ %d,%d,%d", nearest.stationId(), p.getX(), p.getY(), p.getZ())), false);
            return 1;
        }));
    ...
    private static int runVillageAuto(final CommandSourceStack source) {
        ...
        HelloNewGenerationCoreMod.LOGGER.info("[hng-station] villageauto target {},{} surfaceY={} biome={}", target.getX(), target.getZ(), surfaceY, biomeName);
        for (final ResourceLocation poolId : AUTO_START_POOLS) if (tryPlaceVillagePool(source, level, target, poolId)) return 1;
        source.sendSuccess(() -> Component.literal("[hng-station] no village pool could be placed at " + target.getX() + "," + target.getZ()), false);
        return 0;
    }

    private static boolean tryPlaceVillagePool(final CommandSourceStack source, final ServerLevel level, final BlockPos pos, final ResourceLocation poolId) {
        try {
            final ResourceKey<net.minecraft.world.level.levelgen.structure.pools.StructureTemplatePool> key = ResourceKey.create(Registries.TEMPLATE_POOL, poolId);
            final Holder<net.minecraft.world.level.levelgen.structure.pools.StructureTemplatePool> pool = level.registryAccess().registryOrThrow(Registries.TEMPLATE_POOL).getHolder(key).orElse(null);
            if (pool == null) {
                HelloNewGenerationCoreMod.LOGGER.info("[hng-station]   pool {} not found", poolId);
                return false;
            }
            final boolean ok = net.minecraft.world.level.levelgen.structure.pools.JigsawPlacement.generateJigsaw(level, pool, START_JIGSAW, VILLAGE_MAX_DEPTH, pos, false);
            if (!ok) {
                HelloNewGenerationCoreMod.LOGGER.info("[hng-station]   pool {} -> generateJigsaw failed", poolId);
                return false;
            }
            source.sendSuccess(() -> Component.literal("[hng-station] placed village pool " + poolId + " on town edge @ " + pos.getX() + "," + pos.getY() + "," + pos.getZ()), true);
            return true;
    ...
    private static boolean tryPlaceStructure(final CommandSourceStack source, final ServerLevel level, final BlockPos pos, final ResourceLocation structureId) {
        try {
    ...
    private static void placeStart(final ServerLevel level, final ChunkGenerator gen, final StructureStart start) {
        ...
        ChunkPos.rangeClosed(min, max).forEach(cp -> start.placeInChunk(level, level.structureManager(), gen, level.getRandom(), new BoundingBox(cp.getMinBlockX(), level.getMinBuildHeight(), cp.getMinBlockZ(), cp.getMaxBlockX(), level.getMaxBuildHeight(), cp.getMaxBlockZ()), cp));
    }

    private static int placeStructure(final CommandSourceStack source, final ServerLevel level, final BlockPos pos, final ResourceLocation structureId) {
        try {
            final ResourceKey<Structure> key = ResourceKey.create(Registries.STRUCTURE, structureId);
            final Holder<Structure> holder = level.registryAccess().registryOrThrow(Registries.STRUCTURE).getHolder(key).orElse(null);
            if (holder == null) {
            ...
            final Structure structure = holder.value();
            final ChunkGenerator gen = level.getChunkSource().getGenerator();
            final StructureStart start = structure.generate(level.registryAccess(), gen, gen.getBiomeSource(), level.getChunkSource().randomState(), level.getStructureManager(), level.getSeed(), new ChunkPos(pos), 0, level, b -> true);
            if (!start.isValid()) {
                source.sendSuccess(() -> Component.literal("[hng-station] structure failed to generate at " + pos.getX() + "," + pos.getZ() + " (biome/terrain may not allow " + structureId + ")"), false);
                return 0;
            }
            final BoundingBox bb = start.getBoundingBox();
            placeStart(level, gen, start);
            source.sendSuccess(() -> Component.literal(String.format("[hng-station] placed %s, bbox %d,%d,%d -> %d,%d,%d", structureId, bb.minX(), bb.minY(), bb.minZ(), bb.maxX(), bb.maxY(), bb.maxZ())), true);
            return 1;
        } catch (final Throwable t) {
